...
export class AvailabilityService {
    ...
    static async getPropertyAvailability(
        ...
        try {
            ...
            const blockedDateRanges: { start: string; end: string; reason: string }[] = [];
    ...
    static async checkDateRangeAvailability(
        ...
        try {
            console.log('🔍 Checking date range availability:', { propertyId, checkInDate, checkOutDate });
            ...
            console.log('✅ Date range availability:', { available, conflicts: conflictingBookings?.length || 0 });
    ...
    static validateBookingDates(
        ...
        if (checkInDate < minDate) {
            return { valid: false, error: 'Check-in date must be at least 1 day from today' };
        }

        // Check if check-in is too far in the future
        if (checkInDate > maxDate) {
            return { valid: false, error: 'Check-in date cannot be more than 1 year from today' };
        }
