...
const PropertyApproval: React.FC = () => {
  ...
  const fetchProperties = async () => {
    ...
    try {
      ...
      console.log(
        `✅ Successfully fetched ${propertiesData.length} properties`
      );
      ...
      try {
        ...
        if (!enhancedError && enhancedData) {
          ...
          const enhancedMap = new Map(
            enhancedData.map((item) => [item.id, item])
          );
        ...
        console.log(
          "⚠️ Enhanced fields not available, continuing with defaults:",
          enhanceError
        );
      }
      ...
      const propertiesWithOwners = await Promise.all(
        enhancedProperties.map(async (property: any) => {
          try {
            ...
            if (ownerError) {
              console.warn(
                `⚠️ Could not fetch owner for property ${property.id}:`,
                ownerError
              );
            }
  ...
  const handleToggleAdminBlock = async (property: PropertyWithOwner) => {
    try {
      ...
      toast.success(
        `Property ${!property.admin_blocked ? "blocked" : "unblocked"} by admin`
      );
      fetchProperties();
    } catch (error) {
  ...
  const handleToggleMenuAvailable = async (property: PropertyWithOwner) => {
    try {
      ...
      toast.success(
        `Menu ${
          !property.menu_available ? "enabled" : "disabled"
        } for this property`
      );
      fetchProperties();
    } catch (error) {
  ...
  const handleDeleteProperty = async (property: PropertyWithOwner) => {
    if (
      !confirm(
        `Are you sure you want to delete property "${property.title}"? This action cannot be undone.`
      )
    ) {
      return;
    }
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center space-x-4">
              <IconButton
                ...
                onClick={() =>
                  navigate(
                    "/owner/login?switch=1&redirect=/owner/view?tab=properties&add=1"
                  )
                }
                                ...
                                <div className="absolute right-0 top-full mt-1 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
                                  <div className="py-1 flex flex-col">
                                    <IconButton
                                      ...
                                      variant="ghost"
                                      onClick={() =>
                                        handleToggleAdminBlock(property)
                                      }
                                    ...
                                    <IconButton
                                      ...
                                      variant="ghost"
                                      onClick={() =>
                                        handleToggleMenuAvailable(property)
                                      }
                                    ...
                                    <IconButton
                                      ...
                                      variant="ghost"
                                      onClick={() =>
                                        setShowSendNotification(property)
                                      }
